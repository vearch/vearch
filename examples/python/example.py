...
def search_doc_by_filter_of_vearch(vc: Vearch, filters):
    ...
    ret = vc.search("database_test", "book_info", vector_infos=[
                    vi, ], filter=filters, limit=3)
    print("search document", ret.documents)
...
if __name__ == "__main__":
    ...
    dim = 512
    if not space_exist:
        cr_ret_q = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_ivfpq", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, IvfPQIndex("book_vec_idx", 10000, MetricType.Inner_product, 2048, int(dim/4)), dimension=dim)]))
        print(cr_ret_q.__dict__)
        cr_ret_t = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_ivfflat", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, IvfFlatIndex("book_vec_idx", MetricType.Inner_product, 2048), dimension=dim)]))
        print(cr_ret_t.__dict__)

        cr_ret_i = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_biivf", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, BinaryIvfIndex("book_vec_idx", 2048), dimension=512)]))
        print(cr_ret_i.__dict__)

        cr_ret_f = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_flat", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, FlatIndex("book_vec_idx", MetricType.Inner_product), dimension=512)]))
        print(cr_ret_f.__dict__)
        cr_ret_h = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_hnsw", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, HNSWIndex("book_vec_idx", MetricType.Inner_product, 32, 40), dimension=dim)]))
        print(cr_ret_h.__dict__)
        cr_ret_n = create_space(vc, "database_test_not_exist", SpaceSchema("book_info_ivfpq1", [book_name, book_num, Field(
            "book_character", DataType.VECTOR, IvfPQIndex("book_vec_idx", 10000, MetricType.L2, 2048, int(dim/4)), dimension=dim)]))
    ...
    book_name_template = ["qdbwjfwv", "acwlvvq", "cwvwvqqc",
                          "cwvwvveq", "cevqbgnd", "vrhuofnm", "wkvwveve", "cwbtjimu"]
    num = [12, 34, 56, 74, 53, 11, 14, 9]
    data_complete = []
    for i in range(8):
    ...
    ret_q = vc.query("database_test_not_exist",
                     "book_info_ivfpq", document_ids=ids)
    print(ret_q.__dict__, ret_q.code)

    vi = VectorInfo("book_character", [
                    random.uniform(0, 1) for _ in range(512)])
    conditons_search = [
        Condition(operator='>', fv=FieldValue(field="book_num", value=0)),]
    search_filters = Filter(operator="AND", conditions=conditons_search)
    ret_s = vc.search("database_test_not_exist", "book_info_ivfpq",
                      vector_infos=[vi,], filter=search_filters, limit=3)
