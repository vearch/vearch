...
class TreeAHHybridResidual final : public SingleMachineSearcherBase<float> {
 public:
  ...
  virtual ~TreeAHHybridResidual() {
    for (int i = 0; i < leaf_searcher_locks_.size(); ++i) {
  ...
  int AddDataset(shared_ptr<DenseDataset<float>> raw_dataset,
      shared_ptr<DenseDataset<float>> add_dataset) override;

  int AddDatapointsForOneLeaf(std::vector<DatapointIndex> &datapoints_by_token,
      uint32_t leaf_id) override;

  int AddPackedDataset(const shared_ptr<DenseDataset<float>> add_dataset,
      vector<std::vector<DatapointIndex>> &datapoints_by_token,
      shared_ptr<ThreadPool> pool) override;

  int DumpIndex(std::string path) override;
 ...
 private:
  ...
  vector<uint32_t> leafs_indexed_num_;
