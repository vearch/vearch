...
class Searcher final : public SingleMachineSearcherBase<T> {
 public:
  ...
  int AddSearcherPackedDataset(shared_ptr<DenseDataset<uint8_t>> hashed_dataset) override;
                       
  void SetWrlock(pthread_rwlock_t *rw_lock) override {
    rwlock_ = rw_lock;
  }

  uint32_t DatapointsNum() override {
    return packed_dataset_.num_datapoints;
  }
 ...
 private:
  ...
  pthread_rwlock_t *rwlock_ = nullptr;
 public:
  uint8_t extend_time_ = 0;

  PackedDataset packed_dataset_;

  std::vector<uint8_t> hash_ds_;
