...
class SingleMachineSearcherBase : public UntypedSingleMachineSearcherBase {
 public:
  ...
  virtual Status FindNeighbors(const DatapointPtr<T>& query,
                               ...
                               pthread_rwlock_t *rwlock = nullptr) const;

  Status FindNeighbors(const DatapointPtr<T>& query,
                       NNResultsVector* result,
                       pthread_rwlock_t *rwlock = nullptr) const {
    return FindNeighbors(query, default_search_parameters_, result, rwlock);
  }
  ...
  virtual pthread_rwlock_t *GetDatasetRwlock() { return nullptr; }

  virtual int KmeanTokenizeDatabase( 
      shared_ptr<DenseDataset<T>> add_dataset,
      vector<std::vector<DatapointIndex>> &datapoints_by_token,
      ThreadPool* pool) {
    LOG(INFO) << "TreeAHHybridResidual::KmeanTokenizeDatabase";
    return 0;
  }

  virtual int AddDataset(shared_ptr<DenseDataset<T>> raw_dataset,
      shared_ptr<DenseDataset<T>> add_dataset) {
    LOG(INFO) << "SingleMachineSearcherBase::AddDataset";
    return 0;
  }

  virtual int AddDatapointsForOneLeaf(std::vector<DatapointIndex> &datapoints_by_token,
                                      uint32_t bucket_id) {
    LOG(INFO) << "SingleMachineSearcherBase::AddDatapointsForOneLeaf";
    return 0;
  }

  virtual int AddPackedDataset(
      const shared_ptr<DenseDataset<T>> add_dataset,
      vector<std::vector<DatapointIndex>> &datapoints_by_token,
      shared_ptr<ThreadPool> pool) {
    LOG(INFO) << "SingleMachineSearcherBase::AddPackedDataset";
    return 0;
  }

  virtual int AddSearcherPackedDataset(shared_ptr<DenseDataset<uint8_t>> hashed_dataset) {
    LOG(INFO) << "SingleMachineSearcherBase::AddSearcherPackedDataset";
    return 0;
  }

  virtual int DumpIndex(std::string path) {
    LOG(INFO) << "SingleMachineSearcherBase::DumpIndex";
    return 0; 
  }

  virtual int ClearDataset() {
    LOG(INFO) << "SingleMachineSearcherBase::ClearDataset";
    dataset_ = nullptr;
    return 0;
  }

  virtual void SetWrlock(pthread_rwlock_t *rw_lock) {
    LOG(INFO) << "SingleMachineSearcherBase::SetWrlock";
  }

  virtual uint32_t DatapointsNum() {
    LOG(INFO) << "SingleMachineSearcherBase::DatapointsNum";
    return 0;
  }

  double ExtendCoefficient(uint8_t extend_time) {
    return (1.2 + PI / 2 - atan(extend_time));
  }
