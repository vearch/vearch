...
StatusOrSearcherUntyped TreeAhHybridResidualFactory<float>(
  ...
  if (dataset && dataset->empty()) {
    ...
    if (opts->datapoints_by_token) {
      datapoints_by_token.resize(opts->datapoints_by_token->size());
      for (int i = 0; i < datapoints_by_token.size(); ++i) {
        datapoints_by_token[i] = std::move(*((opts->datapoints_by_token)->at(i)));
      }
    } else if (dense) {
  ...
  if (opts->ah_codebook) {
    ...
    TF_ASSIGN_OR_RETURN(
        auto residuals,
        TreeAHHybridResidual::ComputeResiduals(*dense, kmeans_tree_partitioner.get(), datapoints_by_token,
...
StatusOrSearcherUntyped PretrainedSQTreeXHybridFactory(
  ...
  int n_tokens = opts->datapoints_by_token->size();
  if (n_tokens < partitioner->n_tokens()) {
    n_tokens = partitioner->n_tokens();
  }
  vector<std::vector<DatapointIndex>> datapoints_by_token(n_tokens);
...
StatusOrSearcherUntyped NonResidualTreeXHybridFactory(
  ...
  if (opts && opts->datapoints_by_token &&
    ...
    datapoints_by_token.resize(n_tokens);
    for (int i = 0; i < opts->datapoints_by_token->size(); ++i) {
      datapoints_by_token[i] = std::move(*((opts->datapoints_by_token)->at(i)));
    }
  ...
  if (config.hash().has_asymmetric_hash() &&
    ...
    result->SetIndexer(training_results.indexer);
    result->SetnoiseShapingThreshold(training_results.noise_shaping_threshold);
