...
Status ScannInterface::Initialize(
  ...
  if (opts.serialized_partitioner != nullptr) {
    ...
    opts.datapoints_by_token =
        std::make_shared<vector<shared_ptr<vector<DatapointIndex>>>>(
            opts.serialized_partitioner->n_tokens());
    for (int i = 0; i < opts.datapoints_by_token->size(); ++i) {
      opts.datapoints_by_token->at(i) = make_shared<vector<DatapointIndex>>();
    }
...
Status ScannInterface::Initialize(ConstSpan<float> dataset,
  ...
  opts_.parallelization_pool =
      StartThreadPool("scann_threadpool", training_threads - 1);
  return Initialize(InitDataset(dataset, n_points), opts_);
}
...
Status ScannInterface::Initialize(shared_ptr<DenseDataset<float>> dataset,
  ...
  if (config_.has_exact_reordering() == false) {
    scann_->ClearDataset();
    dataset_ = nullptr;
  }
  return OkStatus();
...
Status ScannInterface::Search(const DatapointPtr<float> query,
  ...
  auto rwlock = scann_->GetDatasetRwlock();
  return scann_->FindNeighbors(query, params, res, rwlock);
}
...
Status ScannInterface::SearchBatched(const DenseDataset<float>& queries,
  ...
  auto rwlock = scann_->GetDatasetRwlock();
  return scann_->FindNeighborsBatched(queries, params, MakeMutableSpan(res), rwlock);
}
...
int ScannInterface::AddBatched(std::vector<float> &dataset, uint32_t doc_num) {
  if (doc_num * dimensionality_ != dataset.size()) {
    LOG(INFO) << "dataset.size[" << dataset.size() << "] != doc_num["
              << doc_num << "] * dim[" << dimensionality_ << "]";
    return -1;
  }
  shared_ptr<DenseDataset<float>> add_dataset = 
      make_shared<DenseDataset<float>>(dataset, doc_num);
  
  if (config_.has_exact_reordering()) {
    if (0 != scann_->AddDataset(dataset_, add_dataset)) {
      return -1;
    }
  }
  vector<std::vector<DatapointIndex>> datapoints_by_token;
  scann_->KmeanTokenizeDatabase(add_dataset, datapoints_by_token,
                                opts_.parallelization_pool.get());
  scann_->AddPackedDataset(add_dataset, datapoints_by_token, opts_.parallelization_pool);
  n_points_ += doc_num;
  return 0;
}

int ScannInterface::DumpIndex(std::string path) {
  return scann_->DumpIndex(path);
}
