...
def create_searcher(db, scann_config, training_threads=0):
  return ScannSearcher(scann_config)

if __name__=="__main__":
  def builder_lambda(dataset, config, training_threads, **kwargs):
    return create_searcher(dataset, config, training_threads, **kwargs)



  dataset = np.zeros((10000 * 500, 512), dtype=np.int)
  print("dataset:", dataset.shape)
  scann = ScannBuilder(dataset, 100, "dot_product").set_builder_lambda(builder_lambda).tree(
                       num_leaves=2048, num_leaves_to_search=80, training_sample_size=250000).score_ah(
                       #2, anisotropic_quantization_threshold=0.03125).build()
                       2, anisotropic_quantization_threshold=0.03125).reorder(100).build()
