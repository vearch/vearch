...
def search(engine, xq, k: int, batch: bool):
    ...
    Supports both batch and single query modes.
    """
    results = []
    if batch:
...
def evaluate(engine, xq, gt, k, batch):
    """
    Evaluate the recall of the search results.
    """
    nq = xq.shape[0]
    t0 = time.time()
...
def test_index_flat():
    ...
    # Define table schema
    field_infos = [FieldInfo(name="field_id", data_type=dataType.INT, is_index=True)]  # Add int field for sequence
