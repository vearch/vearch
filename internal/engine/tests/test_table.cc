...
class TableTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    if(utils::isFolderExist("./table"))
      utils::remove_dir("./table");
  ...
  TableInfo *CreateTableInfo(struct Options &opt, bool is_index = false, bool is_long = false) {
    ...
    field_info.data_type = tig_gamma::DataType::INT;
    table->AddField(field_info);
 
    field_info.name = "long";
    field_names.push_back(field_info.name);
    field_info.data_type = tig_gamma::DataType::LONG;
    table->AddField(field_info);
    ...
    field_names.push_back(field_info.name);
    field_info.data_type = tig_gamma::DataType::DOUBLE;
    table->AddField(field_info);
  ...
  int CreateTable(struct Options &opt, bool is_index = false, bool is_long = false) {
  ...
  void CreateValue(int i, std::string &value, tig_gamma::DataType dataype) {
    if (dataype == tig_gamma::DataType::INT) {
      value = std::string((char *)(&i), sizeof(int));
    }  
    if (dataype == tig_gamma::DataType::LONG) {
      long v = (long)i;
      value = std::string((char *)(&v), sizeof(long));
    }
    if (dataype == tig_gamma::DataType::FLOAT) {
      float v = (float)i;
      value = std::string((char *)(&v), sizeof(float));
    }
    if (dataype == tig_gamma::DataType::DOUBLE) {
      double v = (double)i;
      value = std::string((char *)(&v), sizeof(double));
    }
    if (dataype == tig_gamma::DataType::STRING) {
      value = std::to_string(i);
    }
  }

  int GetValue(int i, std::string &value, tig_gamma::DataType datatype) {
    if (datatype == tig_gamma::DataType::INT) {
      int v;
      memcpy(&v, value.data(), sizeof(int));
      return (v != i ? 1 : 0);
    }  
    if (datatype == tig_gamma::DataType::LONG) {
      long v;
      memcpy(&v, value.data(), sizeof(long));
      return (v != i ? 1 : 0);
    }
    if (datatype == tig_gamma::DataType::FLOAT) {
      float v;
      memcpy(&v, value.data(), sizeof(float));
      return (v != i ? 1 : 0);
    }
    if (datatype == tig_gamma::DataType::DOUBLE) {
      double v;
      memcpy(&v, value.data(), sizeof(double));
      return (v != i ? 1 : 0);
    }
    if (datatype == tig_gamma::DataType::STRING) {
  ...
  int BatchAdd(int add_num = 10000) {
    return 0;
  }
};
