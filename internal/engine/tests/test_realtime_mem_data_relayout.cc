...
class RealTimeMemDataRelayoutTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    realtime_data->Relayout(M_, group_size_);
  }
  ...
  int max_vec_size;
  int buckets_num;
  realtime::RealTimeMemDataRelayout *realtime_data;
...
RandomGenerator *RealTimeMemDataRelayoutTest::random_generator = nullptr;

TEST_F(RealTimeMemDataRelayoutTest, AddUpdate) {
...
TEST_F(RealTimeMemDataRelayoutTest, CompactBucket) {
...
TEST_F(RealTimeMemDataRelayoutTest, ExtendBucket) {
  int num = bucket_keys + 1, bucket_no = 0;
  std::vector<long> keys;
  std::vector<uint8_t> codes;
  CreateData(num, keys, codes, code_byte_size);
  ...
  ASSERT_EQ(num, GetRetrievePos(bucket_no));
  ASSERT_EQ(bucket_keys * 2, GetBucketKeys(bucket_no));
  std::vector<long> retrieve_keys;
  std::vector<uint8_t> retrieve_codes;
  retrieve_keys.resize(num);
  retrieve_codes.resize(num * code_byte_size);
...
TEST_F(RealTimeMemDataRelayoutTest, NoMaxSize) {
  int num = bucket_keys;
...
TEST_F(RealTimeMemDataRelayoutTest, DumpLoad) {
  int nums[2];
  nums[0] = bucket_keys;
  ...
  realtime::RTInvertIndexRelayout *rt_invert_index =
      new realtime::RTInvertIndexRelayout(0, 0, nullptr, nullptr);
  rt_invert_index->cur_ptr_ = realtime_data;
  faiss::IOWriter *fw = new FileIOWriter(index_file.c_str());
  ASSERT_EQ(0, WriteInvertedListsRelayout(fw, rt_invert_index));
  delete fw;
  ...
  ASSERT_EQ(0, ReadInvertedListsRelayout(fr, rt_invert_index));
  delete fr;

  // validate
  int expect_nums[2] = {nums[0], nums[1]};
  for (int i = 0; i < 2; i++) {
    expect_nums[i] = nums[i] / group_size_ * group_size_;
  }
