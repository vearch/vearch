...
func parseFilter(filters *request.Filter, space *entity.Space) (*vearchpb.TableFilter, error) {
	rfs := make([]*vearchpb.RangeFilter, 0)
	tfs := make([]*vearchpb.TermFilter, 0)
	var table_filter *vearchpb.TableFilter = &vearchpb.TableFilter{}
	...
	if filters != nil {
		if filters.Operator == "AND" {
			table_filter.Operator = FilterOperatorAnd
		} else if filters.Operator == "OR" {
			table_filter.Operator = FilterOperatorOr
		} else {
			return nil, vearchpb.NewError(vearchpb.ErrorEnum_FILTER_OPERATOR_TYPE_ERR, nil)
		}
		...
		for _, condition := range filters.Conditions {
			if condition.Operator == "<" || condition.Operator == "<=" ||
				...
				if err != nil {
					log.Error(err)
					return nil, vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, err)
				}
				...
				return nil, vearchpb.NewError(vearchpb.ErrorEnum_FILTER_CONDITION_OPERATOR_TYPE_ERR, nil)
			}
		}
		filter, err := parseRange(filters.Operator, rangeConditionMap, proMap)
		if err != nil {
			return nil, vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, fmt.Errorf("parseRange err %s", err.Error()))
		}
		if len(filter) != 0 {
		...
		if err != nil {
			return nil, vearchpb.NewError(vearchpb.ErrorEnum_PARAM_ERROR, fmt.Errorf("parseTerm err %s", err.Error()))
		}
		if len(tmFilter) != 0 {
	...
	if len(rfs) != 0 {
		table_filter.RangeFilters = rfs
	}
	if len(tfs) != 0 {
		table_filter.TermFilters = tfs
	}

	return table_filter, nil
}
...
func parseSearch(vectors []json.RawMessage, filters *request.Filter, req *vearchpb.SearchRequest, space *entity.Space) error {
	...
	table_filter, err := parseFilter(filters, space)
	if err != nil {
		return err
	}
	req.TableFilter = table_filter
...
func queryRequestToPb(searchDoc *request.SearchDocumentRequest, space *entity.Space, queryReq *vearchpb.QueryRequest) error {
	...
	if searchDoc.Filters != nil {
		table_filter, err := parseFilter(searchDoc.Filters, space)
		if err != nil {
			return err
		}
		queryReq.TableFilter = table_filter
	}
