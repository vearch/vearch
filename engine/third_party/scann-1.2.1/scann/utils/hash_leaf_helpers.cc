...
template <typename T>
shared_ptr<DenseDataset<uint8_t>> HashLeafHelpers<T>::GetHashedDataset(
    shared_ptr<TypedDataset<T>> dataset,
    shared_ptr<const asymmetric_hashing2::Indexer<T>> indexer,
  ...
  if (std::isnan(noise_shaping_threshold)) {
    ...
    hashed_dataset = IndexDatabase<T>(
        *dataset,
        [&](const DatapointPtr<T>& dptr, Datapoint<uint8_t>* dp) {
          return indexer->HashWithNoiseShaping(
              dptr, dp, noise_shaping_threshold);
        },
        pool);
  }
